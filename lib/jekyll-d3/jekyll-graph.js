...
export default class JekyllGraph {
  ...
  // d3
  drawNetWeb () {
    fetch('{{ site.baseurl }}{{ site.d3.assets_path }}/graph-net-web.json').then(res => res.json()).then(data => {
  ...
  drawTree () {
    fetch('{{ site.baseurl }}{{ site.d3.assets_path }}/graph-tree.json').then(res => res.json()).then(data => {
